-- Keep the management finance cards, chart and detail drawer on one database
-- snapshot. The function is security-invoker, so existing table RLS still applies.
...
comment on function public.management_finance_snapshot(date) is
  'Single monthly receivables snapshot for management cards, chart, and detail rows; excludes daily bookings and cancelled receivables.';
