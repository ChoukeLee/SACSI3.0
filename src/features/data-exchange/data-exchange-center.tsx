...
export function DataExchangeCenter({ locale, userRole }: Props) {
  ...
  const btn = "rounded-lg border border-brand-warm-400 px-3 py-1.5 text-xs font-medium transition-all bg-white";
  const primaryBtn = "rounded-lg bg-brand-ink-900 px-4 py-2 text-xs font-semibold text-white hover:bg-brand-ink-700 disabled:opacity-50 transition-all";

  return (
    <div className="max-w-4xl mx-auto space-y-4">
      {/* Tabs */}
      <div className="flex gap-1 rounded-lg border border-brand-warm-300 bg-brand-warm-50 p-1 w-fit">
        {(["export", "import"] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={cn("rounded-md px-4 py-1.5 text-[13px] font-medium transition-colors", tab === t ? "bg-white text-brand-ink-900 shadow-sm" : "text-brand-ink-400")}>
      ...
      {tab === "import" && (
        <div className="rounded-xl border border-brand-warm-300 bg-white p-5 shadow-card space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-brand-ink-400 mb-1.5">{locale === "zh" ? "粘贴 CSV 内容" : "Coller CSV"}</label>
            <textarea
              value={impText}
              ...
              className="w-full rounded-lg border border-brand-warm-400 px-3 py-2 text-xs font-mono text-brand-ink-900 focus:outline-none focus:ring-2 focus:ring-brand-orange-500/30"
            />
          </div>
          ...
          {impResult && (
            <div className="space-y-2">
              <div className="flex gap-3 text-xs">
                ...
                <span className="text-amber-600 flex items-center gap-1"><AlertTriangle className="h-3 w-3" />{impResult.warnCount} {locale === "zh" ? "警告" : "alertes"}</span>
              ...
              <div className="overflow-auto max-h-[300px] rounded-lg border border-brand-warm-300 text-xs">
                  ...
                  <tbody className="divide-y divide-brand-warm-200">
                    {impResult.rows.map(r => (
                      <tr key={r.row} className={cn(r.status === "error" ? "bg-brand-red-50/30" : r.status === "warning" ? "bg-brand-amber-50/30" : "")}>
                        ...
                        <td className="px-2 py-1.5 text-brand-ink-500 max-w-[300px] truncate">{Object.values(r.data).join(", ")}</td>
                        <td className={cn("px-2 py-1.5", r.status === "error" ? "text-brand-red-600" : r.status === "warning" ? "text-amber-600" : "text-brand-green-600")}>{r.message}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
