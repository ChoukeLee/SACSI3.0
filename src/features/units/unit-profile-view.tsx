...
function dailyStatusLabel(status: string, locale: Locale) { const zh: Record<string, string> = { pending_review: "待确认", confirmed: "已确认", checked_in: "已入住", checked_out: "已退房", cancelled: "已取消" }; const fr: Record<string, string> = { pending_review: "À confirmer", confirmed: "Confirmé", checked_in: "Arrivé", checked_out: "Parti", cancelled: "Annulé" }; return (locale === "zh" ? zh : fr)[status] ?? status; }
...
function contractStatusLabel(status: ContractStatus, locale: Locale) { const zh: Record<ContractStatus, string> = { active: "生效中", draft: "草稿", terminated: "已终止", expired: "已过期" }; const fr: Record<ContractStatus, string> = { active: "Actif", draft: "Brouillon", terminated: "Résilié", expired: "Expiré" }; return (locale === "zh" ? zh : fr)[status]; }
...
function businessLabel(sourceType: string, category: string | null, locale: Locale) { const zh: Record<string, string> = { daily_booking: "日租", lease_contract: "长租", lease_rent: "长租租金", lease_deposit: "长租押金", sale_contract: "出售", sale_installment: "出售分期", sale_lump_sum: "出售全款", manual: "手工" }; const fr: Record<string, string> = { daily_booking: "Jour", lease_contract: "Bail", lease_rent: "Loyer", lease_deposit: "Dépôt", sale_contract: "Vente", sale_installment: "Échéance vente", sale_lump_sum: "Vente comptant", manual: "Manuel" }; const map = locale === "zh" ? zh : fr; return map[category ?? ""] ?? map[sourceType] ?? sourceType; }
