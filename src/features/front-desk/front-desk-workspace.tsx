...
export function FrontDeskWorkspace({ dailyUnits, bookings, customers, payments, cleaningTasks, locale, buildingName = "SASCI11" }: Props) {
  ...
  const roomColor = (rs: RoomState) => {
    switch (rs.displayStatus) {
      case "occupied": return "bg-brand-red-100 text-brand-red-700 border-brand-red-300";
      case "checking_out_today": return "bg-brand-amber-100 text-amber-700 border-amber-300";
      case "reserved": return "bg-brand-sky-100 text-sky-700 border-sky-300";
      case "cleaning": return "bg-brand-purple-100 text-purple-700 border-purple-300";
      case "available": return "bg-brand-green-100 text-green-700 border-green-300";
      default: return "bg-brand-warm-100 text-brand-ink-400 border-brand-warm-300";
    }
  };
    ...
    <div className="max-w-lg mx-auto pb-8">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={handleCopyShare}
          className={cn("flex items-center gap-1 rounded-lg px-3 py-1.5 text-[11px] font-medium transition-colors", copied ? "bg-brand-green-100 text-brand-green-700" : "bg-brand-warm-100 text-brand-ink-500")}>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <button onClick={() => router.push("/daily-rentals")} className={cn(btnClass, "bg-brand-ink-900 text-white")}>
          <Plus className="h-4 w-4" />{t.newBooking}
        </button>
        <button onClick={() => setMainTab("checkins")} className={cn(btnClass, "bg-brand-sky-50 text-sky-700 border border-sky-200")}>
          <LogIn className="h-4 w-4" />{t.checkins} ({todayCheckins.length})
        </button>
        <button onClick={() => setMainTab("checkouts")} className={cn(btnClass, "bg-brand-amber-50 text-amber-700 border border-amber-200")}>
          <LogOut className="h-4 w-4" />{t.checkouts} ({todayCheckouts.length})
        </button>
        <button onClick={() => setMainTab("occupied")} className={cn(btnClass, "bg-brand-orange-50 text-orange-700 border border-orange-200")}>
          <BedDouble className="h-4 w-4" />{t.occupied} ({occupied.length})
        </button>
        <button onClick={() => setMainTab("pending")} className={cn(btnClass, "bg-brand-warm-100 text-brand-ink-500 border border-brand-warm-300")}>
          <AlertTriangle className="h-4 w-4" />{t.pending} ({pendingBookings.length})
        </button>
        <button onClick={() => router.push("/daily-rentals")} className={cn(btnClass, "bg-brand-green-50 text-green-700 border border-green-200")}>
          <Calendar className="h-4 w-4" />{t.roomStatus}
        </button>
      </div>

      {/* Room matrix — apartments only, compact */}
      <div className="mb-4 rounded-xl border border-brand-warm-300 bg-white p-3">
        <h3 className="text-[11px] font-bold text-brand-ink-500 mb-2">{t.roomStatus} ({roomStates.length})</h3>
        ...
        <div className="mt-2 flex flex-wrap gap-x-3 gap-y-0.5 text-[9px] text-brand-ink-400">
          ...
          <span className="flex items-center gap-1"><span className="h-2 w-2 rounded bg-sky-400" />{t.reserved} ({reserved.length})</span>
        </div>
      </div>

      {/* Tab lists */}
      <div className="rounded-xl border border-brand-warm-300 bg-white overflow-hidden">
        <div className="flex border-b border-brand-warm-200 text-[11px] font-medium">
          {(["checkins", "occupied", "checkouts", "pending"] as MainTab[]).map(tab => (
            <button key={tab}
              onClick={() => setMainTab(tab)}
              className={cn("flex-1 py-2.5 text-center transition-colors", mainTab === tab ? "bg-brand-orange-50 text-brand-orange-700 border-b-2 border-brand-orange" : "text-brand-ink-400")}>
        ...
        <div className="divide-y divide-brand-warm-100 max-h-[320px] overflow-auto">
          {(mainTab === "checkins" ? todayCheckins :
            ...
            return (
              <div key={b.id} className="flex items-center gap-2 px-3 py-2.5 text-xs">
                <span className="font-mono font-bold text-brand-ink-700 min-w-[32px]">{unit?.unit_no ?? "?"}</span>
                ...
                <span className={cn("rounded-full px-1.5 py-0.5 text-[9px] font-semibold",
                  ...
                  b.status === "confirmed" ? "bg-brand-sky-100 text-sky-700" :
                  b.status === "pending_review" ? "bg-brand-amber-100 text-amber-700" :
                  "bg-brand-warm-100 text-brand-ink-400"
                )}>
                ...
                <div className="flex gap-1">
                  {b.status === "pending_review" && (
                    <button onClick={() => openRoomAction(rs!, "confirm")}
                      className="rounded bg-brand-sky-500 px-2 py-1 text-[9px] font-semibold text-white">{t.confirm}</button>
                  )}
                  {b.status === "confirmed" && b.check_in === todayStr && (
                    <button onClick={() => openRoomAction(rs!, "checkin")}
                      className="rounded bg-brand-green-500 px-2 py-1 text-[9px] font-semibold text-white">{t.checkin}</button>
                  )}
                  {b.status === "checked_in" && (
                    <button onClick={() => openRoomAction(rs!, "checkout")}
                      className="rounded bg-brand-amber-500 px-2 py-1 text-[9px] font-semibold text-white">{t.checkout}</button>
                  )}
                </div>
              </div>
            );
          ...
          {((mainTab === "checkins" && todayCheckins.length === 0) ||
            ...
            <div className="py-10 text-center text-xs text-brand-ink-300">{t.noData}</div>
          )}
        </div>
      </div>
      ...
      {selectedRoom && !popupAction && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={() => setSelectedRoom(null)}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-sm max-h-[70vh] overflow-auto shadow-xl border border-brand-warm-300 p-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-bold text-brand-ink-900">{selectedRoom.unit.unit_no} · {selectedRoom.unit.floor_label}</h3>
              <button onClick={() => setSelectedRoom(null)} className="p-1 rounded hover:bg-brand-warm-100"><X className="h-4 w-4 text-brand-ink-400" /></button>
            </div>
            {selectedRoom.customer && (
              <div className="space-y-1 text-xs mb-3">
                <p className="flex items-center gap-1"><User className="h-3 w-3 text-brand-ink-400" />{selectedRoom.customer.name}</p>
                {selectedRoom.customer.phone && <p className="flex items-center gap-1"><Phone className="h-3 w-3 text-brand-ink-400" />{selectedRoom.customer.phone}</p>}
              </div>
            )}
            {selectedRoom.booking ? (
              <div className="space-y-1 text-xs mb-3">
                <p><span className="text-brand-ink-400">{t.date}:</span> {selectedRoom.booking.check_in} → {selectedRoom.booking.check_out ?? (locale === "zh" ? "未定" : "?")}</p>
                <p><span className="text-brand-ink-400">{t.amount}:</span> {formatXof(Number(selectedRoom.booking.total_amount_xof))}</p>
                <p><span className="text-brand-ink-400">{t.prepaid}:</span> <span className="text-brand-green-600">{formatXof(selectedRoom.totalPaid)}</span></p>
                {selectedRoom.billing && selectedRoom.billing.finalAmount > selectedRoom.totalPaid && (
                  <p><span className="text-brand-ink-400">{t.due}:</span> <span className="text-brand-red-600 font-semibold">{formatXof(selectedRoom.billing.finalAmount - selectedRoom.totalPaid)}</span></p>
                )}
              </div>
            ) : (
              <p className="text-xs text-brand-ink-300 mb-3">{t.available}</p>
            )}
            <div className="grid grid-cols-2 gap-2">
              {(!selectedRoom.booking || selectedRoom.booking.status === "pending_review") && (
                <button onClick={() => openRoomAction(selectedRoom, "confirm")} className={cn(btnClass, "bg-brand-sky-50 text-sky-700")}>{t.confirm}</button>
              )}
              ...
              {selectedRoom.booking?.status === "checked_in" && (
                <>
                  ...
                  <button onClick={() => openRoomAction(selectedRoom, "payment")} className={cn(btnClass, "bg-brand-orange-50 text-orange-700")}>{t.payment}</button>
                </>
              )}
              {selectedRoom.booking?.status && ["pending_review", "confirmed"].includes(selectedRoom.booking.status) && (
                <button onClick={() => openRoomAction(selectedRoom, "cancel")} className={cn(btnClass, "bg-brand-red-50 text-red-600")}>{t.cancel}</button>
              )}
            </div>
          </div>
        </div>
      ...
      {popupAction && selectedRoom && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={() => { setPopupAction(null); setSelectedRoom(null); }}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-sm shadow-xl border border-brand-warm-300 p-4" onClick={e => e.stopPropagation()}>
            ...
            {popupAction === "checkin" && (
              <div className="space-y-2 mb-3">
                <label className="text-[10px] text-brand-ink-400">{t.prepaid}</label>
                <input type="number" value={popupAmount} onChange={e => setPopupAmount(Number(e.target.value))}
                  className="w-full rounded-lg border border-brand-warm-400 px-3 py-2 text-sm" />
              </div>
            )}
            {popupAction === "payment" && (
              <div className="space-y-2 mb-3">
                <label className="text-[10px] text-brand-ink-400">{t.amount}</label>
                <input type="number" value={popupAmount} onChange={e => setPopupAmount(Number(e.target.value))}
                  className="w-full rounded-lg border border-brand-warm-400 px-3 py-2 text-sm" />
                <label className="text-[10px] text-brand-ink-400">{t.receiptNo}</label>
            ...
            <div className="flex gap-2">
              <button onClick={() => { setPopupAction(null); setSelectedRoom(null); }}
                className="flex-1 rounded-lg border border-brand-warm-400 py-2.5 text-sm font-medium text-brand-ink-500">{t.closeModal}</button>
              <button onClick={doAction} disabled={loading}
                className="flex-1 rounded-lg bg-brand-ink-900 py-2.5 text-sm font-semibold text-white disabled:opacity-50">
      ...
      {/* Toast for copy */}
      {copied && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-brand-ink-900 text-white rounded-full px-5 py-2 text-xs font-medium shadow-lg animate-pulse">
