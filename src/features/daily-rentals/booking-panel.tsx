...
export function BookingPanel({ booking, unitId, defaultDate, units, customers, cleaningTasks, payments, locale, onClose, onChanged, onBookingCreated }: BookingPanelProps) {
  ...
  const inputClass = "w-full rounded-lg border border-brand-warm-200 bg-white px-3 py-2.5 text-sm text-brand-ink-900 transition-all duration-fast hover:border-brand-neutral-500 focus:outline-none focus:ring-2 focus:ring-brand-orange-500/30";
  const labelClass = "block text-[10px] font-black uppercase tracking-[0.14em] text-brand-ink-400 mb-1";
          ...
          {isNew && (<>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button onClick={() => setNewCheckoutMode("fixed")} className={cn("flex-1 rounded-lg border px-3 py-2.5 text-xs font-semibold transition-all duration-fast", newCheckoutMode === "fixed" ? "border-brand-orange bg-brand-orange-50 text-brand-orange-700" : "border-brand-warm-200 bg-white text-brand-ink-600 hover:bg-brand-warm-50")}>{t.fixedCheckout}</button>
                <button onClick={() => setNewCheckoutMode("open")} className={cn("flex-1 rounded-lg border px-3 py-2.5 text-xs font-semibold transition-all duration-fast", newCheckoutMode === "open" ? "border-brand-orange bg-brand-orange-50 text-brand-orange-700" : "border-brand-warm-200 bg-white text-brand-ink-600 hover:bg-brand-warm-50")}>{t.openCheckout}</button>
              </div>
            </div>
          ...
          {booking && (<>
            <div className="flex items-start justify-between">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-semibold", booking.checkout_mode === "open" ? "bg-brand-amber-100 text-brand-amber-700" : "bg-brand-warm-100 text-brand-ink-600")}>{booking.checkout_mode === "open" ? t.openEndedBadge : t.fixedBadge}</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 rounded-lg bg-brand-warm-50 p-3 text-sm">
              <div><p className="text-[10px] text-brand-ink-400">{t.booking.checkInDate}</p><p className="font-semibold text-brand-ink-900">{booking.check_in}</p></div>
              <div><p className="text-[10px] text-brand-ink-400">{booking.checkout_mode === "open" ? t.actualCheckOutDate : t.booking.checkOutDate}</p><p className="font-semibold text-brand-ink-900">{booking.checkout_mode === "open" ? (booking.actual_check_out ?? "—") : booking.check_out}</p></div>
            </div>
            ...
            <div className="space-y-2">
                ...
                <div className="space-y-3">
                  ...
                  {booking.checkout_mode === "fixed" && (
                    <div className="rounded-lg border border-brand-warm-200 bg-brand-warm-50 p-3">
                      ...
                      <div className="flex items-center gap-2">
                        <input type="number" min={1} value={extendDays} onChange={e => setExtendDays(e.target.value)} className="w-16 rounded-lg border border-brand-warm-200 px-2 py-2 text-sm transition-all duration-fast hover:border-brand-neutral-500 focus:outline-none focus:ring-2 focus:ring-brand-orange-500/30" />
              ...
              {booking.status === "checked_out" && (
                <div className="rounded-lg bg-brand-warm-50 p-3 text-center text-sm text-brand-ink-600">{t.bookingStatus.checked_out}{relatedCleaningTask && !relatedCleaningTask.is_completed && <p className="mt-1 text-xs text-brand-sky-600">{t.cleaning.pending}</p>}</div>
              )}
            </div>
