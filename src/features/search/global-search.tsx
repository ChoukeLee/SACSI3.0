...
export function GlobalSearch({ locale }: { locale: "zh" | "fr" }) {
  ...
  if (!open) return (
    <>
      ...
      <button
        ...
        className="hidden sm:flex items-center gap-2 rounded-lg border border-brand-warm-300 bg-brand-warm-50 px-3 py-1.5 text-xs text-brand-ink-400 hover:bg-brand-warm-100 hover:text-brand-ink-600 transition-colors"
      >
        <Search className="h-3.5 w-3.5" />
        <span className="hidden lg:inline">{locale === "zh" ? "搜索..." : "Rechercher..."}</span>
        <kbd className="hidden lg:inline rounded border border-brand-warm-300 bg-white px-1.5 py-0 text-[10px] font-mono text-brand-ink-300">Ctrl+K</kbd>
      </button>
      {/* Mobile search icon */}
      <button
        onClick={() => setOpen(true)}
        className="flex sm:hidden items-center justify-center rounded-lg border border-brand-warm-300 bg-brand-warm-50 p-1.5 text-brand-ink-400 hover:bg-brand-warm-100"
      >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]" onClick={() => setOpen(false)}>
      ...
      <div
        className="relative w-full max-w-lg rounded-2xl border border-brand-warm-300 bg-white shadow-2xl overflow-hidden mx-4"
        onClick={e => e.stopPropagation()}
      >
        {/* Input */}
        <div className="flex items-center gap-2 px-4 py-3 border-b border-brand-warm-200">
          <Search className="h-5 w-5 shrink-0 text-brand-ink-300" />
          <input
            ref={inputRef}
            type="text"
            value={query}
            ...
            className="flex-1 text-sm text-brand-ink-900 placeholder:text-brand-ink-300 bg-transparent outline-none"
          />
          {loading && <Loader2 className="h-4 w-4 animate-spin text-brand-ink-300" />}
        ...
        <div className="max-h-[360px] overflow-auto">
          {query.length < 2 ? (
            ...
            <div className="py-8 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-brand-ink-300" /></div>
          ) : results.length === 0 ? (
            <div className="py-12 text-center text-sm text-brand-ink-300">
            ...
            [...grouped.entries()].map(([type, items]) => (
              <div key={type}>
                <div className="px-4 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-brand-ink-300 bg-brand-warm-50">
                ...
                {items.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className="h-4 w-4 shrink-0 text-brand-ink-400" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-brand-ink-800 truncate">{r.title}</p>
                        <p className="text-xs text-brand-ink-400 truncate">
        ...
        {/* Footer */}
        <div className="flex items-center gap-3 px-4 py-2 border-t border-brand-warm-200 text-[10px] text-brand-ink-300">
