...
export function CustomerProfileView({ data, locale, userRole }: Props) {
  ...
  const inputClass = "w-full rounded border border-brand-warm-400 bg-white px-3 py-2 text-sm";
  const labelClass = "text-[10px] text-brand-ink-400";
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-xl font-bold text-brand-ink-900">{customer.name}</h1>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-brand-ink-500">
              ...
              {customer.notes && <span className="text-brand-ink-300">{customer.notes}</span>}
            </div>
          </div>
      ...
      <div className="flex gap-1 rounded-lg border border-brand-warm-300 bg-brand-warm-50 p-1 overflow-x-auto">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={cn("shrink-0 rounded-md px-3 py-1.5 text-[12px] font-medium transition-colors", tab === t.key ? "bg-white text-brand-ink-900 shadow-sm" : "text-brand-ink-400 hover:text-brand-ink-600")}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === "overview" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            ...
            <div className="rounded-lg border border-brand-warm-300 bg-white p-3 text-xs">
              <p className="text-brand-ink-400">{L.lastBooking}</p>
            ...
            <div className="rounded-lg border border-brand-warm-300 bg-white p-3 text-xs">
              <p className="text-brand-ink-400">{locale === "zh" ? "当前业务" : "Actif"}</p>
          ...
          {data.units.length > 0 && (
            <div className="rounded-lg border border-brand-warm-300 bg-white p-3">
              <p className="text-xs font-semibold text-brand-ink-700 mb-2">{locale === "zh" ? "关联房源" : "Logements lies"}</p>
              <div className="flex flex-wrap gap-1.5">
                {data.units.map(u => (
                  <span key={u.id} className="rounded bg-brand-warm-100 px-2 py-0.5 text-[10px] font-mono text-brand-ink-600">{u.unit_no} ({u.kind})</span>
                ))}
              </div>
            </div>
          )}
      ...
      {tab === "daily" && (
        <div className="rounded-xl border border-brand-warm-300 bg-white overflow-hidden">
          {data.dailyBookings.length === 0 ? (
            <div className="py-10 text-center text-sm text-brand-ink-300">{L.noData}</div>
          ) : (
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-brand-warm-200">
                {data.dailyBookings.map(b => {
                  const unit = data.units.find(u => u.id === b.unit_id);
                  return (<tr key={b.id} className="hover:bg-brand-warm-50">
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-brand-warm-200">
                {data.leaseContracts.map(lc => {
                  ...
                  return (<tr key={lc.id} className="hover:bg-brand-warm-50">
                    <td className="px-3 py-2 font-mono font-medium">{unit?.unit_no ?? "?"}</td>
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-brand-warm-200">
                {data.saleContracts.map(sc => {
                  ...
                  const statusV: Record<string, "neutral" | "success" | "danger" | "warning"> = { active: "success", draft: "neutral", terminated: "danger", expired: "warning" };
                  const statusL: Record<string, string> = locale === "zh" ? { active: "生效", draft: "草稿", terminated: "终止", expired: "过期" } : { active: "Actif", draft: "Brouillon", terminated: "Resilie", expired: "Expire" };
                  return (<tr key={sc.id} className="hover:bg-brand-warm-50">
                    <td className="px-3 py-2 font-mono font-medium">{unit?.unit_no ?? "?"}</td>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(["daily_booking", "lease_contract", "sale_contract"] as const).map(src => {
              ...
              return (
                <div key={src} className="rounded-lg border border-brand-warm-300 bg-white p-3 text-xs">
                  <p className="text-brand-ink-400">{srcLabels[src]}</p>
                  <p className="font-semibold">{formatXof(paid)} <span className="text-brand-ink-300 font-normal">/ {formatXof(total)}</span></p>
                </div>
              );
            })}
          </div>
          {/* Receivables */}
          <div className="rounded-xl border border-brand-warm-300 bg-white overflow-hidden">
            <p className="px-3 py-2 text-[11px] font-bold text-brand-ink-700 bg-brand-warm-50">{locale === "zh" ? "应收记录" : "Creances"} ({data.receivables.filter(r => r.status !== "cancelled").length})</p>
            <table className="w-full text-xs">
              <thead className="bg-brand-warm-50/50 text-[10px] uppercase text-brand-ink-400"><tr>
              ...
              <tbody className="divide-y divide-brand-warm-200">
                {data.receivables.filter(r => r.status !== "cancelled").slice(0, 50).map(r => {
                  ...
                  return (<tr key={r.id} className={cn("hover:bg-brand-warm-50", r.status === "overdue" && "bg-brand-red-50/30")}>
                    <td className="px-3 py-1.5">{r.due_date}</td><td className="px-3 py-1.5 text-brand-ink-500">{srcLabels[r.source_type] ?? r.source_type}</td>
          ...
          {/* Payments */}
          <div className="rounded-xl border border-brand-warm-300 bg-white overflow-hidden">
            <p className="px-3 py-2 text-[11px] font-bold text-brand-ink-700 bg-brand-warm-50">{locale === "zh" ? "收款记录" : "Paiements"} ({data.payments.length})</p>
            <table className="w-full text-xs">
              <thead className="bg-brand-warm-50/50 text-[10px] uppercase text-brand-ink-400"><tr>
              ...
              <tbody className="divide-y divide-brand-warm-200">
                {data.payments.slice(0, 50).map(p => (
                  <tr key={p.id} className="hover:bg-brand-warm-50">
                    ...
                    <td className="px-3 py-1.5 text-brand-ink-400">{p.receipt_no ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-xl border border-brand-warm-300 bg-white overflow-hidden">
          {customerDocs.length === 0 ? (
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-brand-warm-200">
                {customerDocs.map(d => (
                  <tr key={d.id} className="hover:bg-brand-warm-50">
                    <td className="px-3 py-2 font-medium">{d.title}</td>
                    <td className="px-3 py-2 text-brand-ink-500">{d.date}</td>
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-brand-warm-200">
                {data.auditLogs.slice(0, 50).map(l => {
                  ...
                  return (<tr key={l.id} className="hover:bg-brand-warm-50">
                    <td className="px-3 py-2 text-[10px] text-brand-ink-400">{new Date(l.created_at).toLocaleDateString(locale === "zh" ? "zh-CN" : "fr-FR")}</td>
                    <td className="px-3 py-2">{actionLabels[l.action] ?? l.action}</td>
                    <td className="px-3 py-2 text-brand-ink-400">{l.entity_type} {l.entity_id?.slice(0, 8)}</td>
                  </tr>);
                })}
              </tbody>
            </table>
