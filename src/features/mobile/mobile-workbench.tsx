...
export function MobileWorkbench({ dailyUnits, bookings, customers, payments, cleaningTasks, notifications, locale }: MobileWorkbenchProps) {
  ...
  const statBadge = (n: number) => n > 0 ? "inline-flex h-6 w-6 items-center justify-center rounded-full bg-brand-orange-500 text-xs font-bold text-white" : "";
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className={cn(cardClass, "text-center")}>
          <BedDouble className="mx-auto h-5 w-5 text-brand-orange-500" />
        ...
        <div className={cn(cardClass, "text-center")}>
          <DoorOpen className="mx-auto h-5 w-5 text-blue-500" />
        ...
        <div className={cn(cardClass, "text-center")}>
          <CheckCircle className="mx-auto h-5 w-5 text-sky-500" />
        ...
        <div className={cardClass}>
          ...
          <div className="space-y-2">
            {todayData.occupied.map((r, i) => (
              <div key={i} className="flex items-center justify-between rounded-lg bg-brand-ink-50 px-3 py-2.5">
                <div>
                ...
                <div className="text-right text-xs text-brand-ink-500">
                  ...
                  {r.outstanding > 0 && <div className="font-bold text-red-600">{formatXof(r.outstanding)}</div>}
                  {r.mode === "open" && <div className="text-amber-600">{locale === "zh" ? "未定" : "?"}</div>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className={cn(cardClass, "border-amber-200 bg-amber-50/50")}>
          ...
          {todayData.needTopUp.map((r, i) => (
            <div key={i} className="flex items-center justify-between text-sm">
              ...
              <span className="font-bold text-red-600">{formatXof(r.outstanding)}</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div className={cardClass}>
          ...
          <div className="flex flex-wrap gap-2">
            {todayData.cleaning.map((c, i) => (
              <span key={i} className="rounded-full bg-sky-100 px-3 py-1 text-xs font-medium text-sky-700">{c.unitNo}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {urgentNotifs.length > 0 && (
        <div className={cardClass}>
          ...
          {urgentNotifs.map((n, i) => (
            <div key={i} className="flex items-start gap-2 rounded-lg bg-brand-ink-50 px-3 py-2 mb-1.5">
              <AlertTriangle className="h-4 w-4 shrink-0 text-brand-orange-500 mt-0.5" />
              <div>
