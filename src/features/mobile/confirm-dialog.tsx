...
export function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-overlay flex items-end sm:items-center justify-center">
      <div className="absolute inset-0 bg-brand-ink-900/30" onClick={onClose} />
      <div className="relative w-full max-w-sm rounded-t-2xl sm:rounded-2xl bg-white p-5 shadow-panel animate-in slide-in-from-bottom-4 duration-fast">
        <button
          onClick={onClose}
          className="absolute right-4 top-4 rounded-full p-1 text-brand-ink-300 hover:bg-brand-warm-100"
          aria-label={t.cancel}
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            {description && (
              <p className="mt-1 text-xs text-brand-ink-500">{description}</p>
            )}
          </div>
        </div>
