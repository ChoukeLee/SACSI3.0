      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3">
        <div className="flex items-center gap-2.5">
          <span className={cn(
            ...
            high.length > 0 ? "bg-rose-100 text-rose-600" : "bg-emerald-100 text-brand-green-600",
          )}>
            <ShieldAlert className="h-4 w-4" />
          </span>
          <div>
            <h3 className="text-sm font-black text-slate-950">{locale === "zh" ? "数据健康" : "Sante donnees"}</h3>
      ...
      <div className="bg-slate-50/60 px-4 py-4">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2">
          ...
          <p className={cn(
            ...
            high.length > 0 ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-brand-green-700",
          )}>
            {high.length > 0
...
function QualityMetric({
  ...
  const styles = {
    ...
    emerald: "border-emerald-300 bg-brand-green-500 text-white",
  }[tone];
