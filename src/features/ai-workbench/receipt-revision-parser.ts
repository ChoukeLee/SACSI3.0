...
export function parseReceiptRevisionInstruction(instruction: string): ReceiptRevisionPatch {
  ...
  const labelledAmount = text.match(/(?:金额|款项|总额|montant)\s*(?:改成|改为|调整为|为|:|：|à)?\s*([\d.,]+)\s*(万|千|w|k|xof|西法)?/i);
  const currencyAmount = labelledAmount ?? text.match(/([\d.,]+)\s*(万|千|w|k|xof|西法)(?:\b|元)?/i);
  if (currencyAmount) {
  ...
  if (/(?:取消|清空|去掉).{0,5}(?:已缴至|缴至|付至|pay[ée]? jusqu)/i.test(text)) patch.paidThroughDate = null;
  ...
  if (/(?:物业费|物管费|charges?)/i.test(text)) patch.businessHint = "property_fee";
  else if (/(?:租金|loyer)/i.test(text)) patch.businessHint = "rent";
  ...
  if (/(?:清空|删除|去掉).{0,5}(?:备注|note)/i.test(text)) patch.notes = null;
