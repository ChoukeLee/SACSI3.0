  ...
  return (
    <div>
      ...
      <div className="flex gap-4">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filtered.map(d => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-slate-500 whitespace-nowrap">{d.date}</td>
                      <td className="px-3 py-2 font-mono text-slate-700">{d.unitNo || "—"}</td>
                      <td className="px-3 py-2 text-slate-700 max-w-[80px] truncate">{d.customerName || "—"}</td>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="rounded p-1 text-slate-400 hover:text-brand-orange hover:bg-slate-50/80"
        ...
        {/* Preview panel */}
        {previewed && (
          <div className="w-[380px] shrink-0 rounded-xl border border-slate-200 bg-white shadow-natural flex flex-col">
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              <h3 className="text-sm font-black text-slate-950 flex items-center gap-1.5">
                <FileText className="h-4 w-4 text-brand-orange" />
                {locale === "zh" ? "单据预览" : "Apercu"}
              </h3>
              <button onClick={() => setPreviewId(null)} className="rounded p-1 text-slate-400 hover:bg-slate-50/80">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="flex-1 overflow-auto p-4 space-y-3">
              <div className="text-center border-b border-slate-200 pb-3">
                <p className="text-[10px] uppercase tracking-[0.14em] text-brand-orange font-semibold">SACIS 3.0 · {locale === "zh" ? "科建地产" : "Kejian Immobilier"}</p>
                <p className="text-sm font-black text-slate-950 mt-1">{typeLabels[previewed.docType]}</p>
              </div>
              <dl className="space-y-2 text-xs">
                <div className="flex justify-between"><dt className="text-slate-500">{locale === "zh" ? "标题" : "Libelle"}</dt><dd className="font-medium text-slate-800">{previewed.title}</dd></div>
                <div className="flex justify-between"><dt className="text-slate-500">{locale === "zh" ? "日期" : "Date"}</dt><dd className="text-slate-700">{previewed.date}</dd></div>
                <div className="flex justify-between"><dt className="text-slate-500">{locale === "zh" ? "房号" : "Chambre"}</dt><dd className="font-mono text-slate-800">{previewed.unitNo || "—"}</dd></div>
                <div className="flex justify-between"><dt className="text-slate-500">{locale === "zh" ? "客户" : "Client"}</dt><dd className="text-slate-800">{previewed.customerName || "—"}</dd></div>
                {previewed.customerPhone && <div className="flex justify-between"><dt className="text-slate-500">{locale === "zh" ? "电话" : "Tel"}</dt><dd className="text-slate-700">{previewed.customerPhone}</dd></div>}
                {previewed.contractNo && <div className="flex justify-between"><dt className="text-slate-500">{locale === "zh" ? "合同号" : "N° contrat"}</dt><dd className="font-mono text-slate-700">{previewed.contractNo}</dd></div>}
                <div className="flex justify-between border-t border-slate-200 pt-2"><dt className="text-slate-500">{locale === "zh" ? "金额" : "Montant"}</dt><dd className="font-semibold text-slate-950">{formatXof(previewed.amountXof)}</dd></div>
                {previewed.amountXof > 0 && (
                  <div className="flex justify-between">
                    <dt className="text-slate-500">{locale === "zh" ? "已收" : "Paye"}</dt>
                    <dd className={cn("font-semibold", previewed.paidAmountXof > 0 ? "text-brand-green-700" : "text-slate-400")}>{formatXof(previewed.paidAmountXof)}</dd>
                  </div>
                )}
                ...
                <div className="flex justify-between"><dt className="text-slate-500">{locale === "zh" ? "状态" : "Statut"}</dt><dd>{statusBadge(previewed.status)}</dd></div>
              </dl>
              <div className="border-t border-slate-200 pt-3">
                <button
                  onClick={() => handlePrint(previewed)}
                  className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-slate-800 active:scale-[0.98]"
                >
                  <Printer className="h-4 w-4" />{locale === "zh" ? "打印单据" : "Imprimer"}
                </button>
                <p className="mt-1.5 text-[10px] font-semibold text-slate-400 text-center">
