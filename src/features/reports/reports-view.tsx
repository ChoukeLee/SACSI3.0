    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1 rounded-lg border border-brand-warm-300 bg-brand-warm-50 p-1 overflow-x-auto">
        {tabs.filter(t => permitted.includes(t.key)).map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} className={cn("shrink-0 rounded-md px-3 py-1.5 text-[12px] font-medium transition-colors", tab===t.key?"bg-white text-brand-ink-900 shadow-sm":"text-brand-ink-400")}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {Object.entries(roomStatusData.statuses).map(([k,v]) => {
              ...
              return statBox(labels[k]??k, String(v), k==="available"?"green":k==="daily_occupied"?"red":"ink");
            })}
          </div>
          <button onClick={() => downloadCsv("类型,状态,数量",Object.entries(roomStatusData.statuses).map(([k,v])=>csvLine([k,(locale==="zh"?{available:"空闲",daily_occupied:"日租中",leased:"长租中",sold:"已售"}:{available:"Dispo",daily_occupied:"Occupe",leased:"Loue",sold:"Vendu"})[k]??k,v])),"room_status")}
            className="rounded-lg border border-brand-warm-400 px-3 py-1.5 text-xs font-medium text-brand-ink-600 hover:bg-brand-warm-100 inline-flex items-center gap-1.5"><Download className="h-3.5 w-3.5"/>{L.exportCsv}</button>
        </div>
      )}
      ...
      {tab === "overdue" && (
        <div className="space-y-2">
          <div className="flex gap-2 text-xs text-brand-ink-400">{locale==="zh"?"共":"Total"}: {overdueData.length} {locale==="zh"?"条":"lignes"} · {locale==="zh"?"欠费合计":"Impaye"}: {formatXof(overdueData.reduce((s,r)=>s+r.unpaid,0))}</div>
          {overdueData.length===0 ? <p className="text-sm text-brand-ink-300 py-8 text-center">{L.noData}</p> : (
            <div className="rounded-xl border border-brand-warm-300 bg-white overflow-auto max-h-[500px]">
              <table className="w-full text-xs"><thead className="bg-brand-warm-50 text-[10px] uppercase text-brand-ink-400 sticky top-0"><tr><th className="px-3 py-2">{locale==="zh"?"房号":"Ch"}</th><th className="px-3 py-2">{locale==="zh"?"客户":"Client"}</th><th className="px-3 py-2 text-right">{locale==="zh"?"欠费":"Impaye"}</th><th className="px-3 py-2 text-right">{locale==="zh"?"逾期天数":"Jours"}</th><th className="px-3 py-2">{locale==="zh"?"到期":"Echeance"}</th></tr></thead>
              <tbody className="divide-y divide-brand-warm-200">{overdueData.map(r => <tr key={r.id} className="bg-brand-red-50/20"><td className="px-3 py-2 font-mono">{r.unitNo}</td><td className="px-3 py-2">{r.cust}</td><td className="px-3 py-2 text-right font-semibold text-brand-red-600">{formatXof(r.unpaid)}</td><td className="px-3 py-2 text-right text-brand-red-500">+{r.overdueDays}</td><td className="px-3 py-2">{r.due_date}</td></tr>)}</tbody></table>
            </div>
          )}
      ...
      {tab === "lease" && (
        <div className="space-y-4">
          ...
          {leaseData.expiring30d.length>0 && <div className="text-xs"><p className="font-semibold text-brand-ink-700 mb-1">{locale==="zh"?"30天内到期合同":"Expirent sous 30j"}:</p>{leaseData.expiring30d.map(l => <p key={l.id} className="text-brand-ink-500">{l.contract_no} — {l.expected_end_date}</p>)}</div>}
      ...
      {tab === "daily_close" && (
        <div className="space-y-4">
          ...
          }} className="rounded-lg bg-brand-ink-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-brand-ink-700 inline-flex items-center gap-1.5">
          ...
          <button onClick={() => downloadCsv("指标,数值",[["新预订",dailyCloseData.newBookings.length],["入住",dailyCloseData.checkins.length],["退房",dailyCloseData.checkouts.length],["在住",dailyCloseData.inHouse.length],["收款",dailyCloseData.todayTotal]].map(r=>csvLine(r)),"daily_close")}
            className="rounded-lg border border-brand-warm-400 px-3 py-1.5 text-xs font-medium text-brand-ink-600 hover:bg-brand-warm-100 inline-flex items-center gap-1.5 ml-2"><Download className="h-3.5 w-3.5"/>{L.exportCsv}</button>
        </div>
      )}
    </div>
  );
