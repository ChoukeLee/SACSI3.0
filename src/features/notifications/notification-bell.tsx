  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative rounded-lg p-1.5 text-brand-ink-400 transition hover:bg-brand-warm-100 hover:text-brand-ink-600"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-xl border border-brand-warm-400 bg-white shadow-panel">
            <div className="flex items-center justify-between border-b border-brand-warm-400 px-4 py-3">
              ...
              <div className="flex items-center gap-1">
                {unread > 0 && (
                  <button
                    onClick={markAllRead}
                    className="rounded px-2 py-0.5 text-xs text-brand-ink-300 hover:bg-brand-warm-100 hover:text-brand-ink-600"
                  >
                ...
                <button
                  ...
                  className="rounded p-0.5 text-brand-ink-300 hover:text-brand-ink-600"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
            ...
            <div className="max-h-80 overflow-auto">
              {items.length === 0 ? (
                <p className="px-4 py-8 text-center text-xs text-brand-ink-300">{t.empty}</p>
              ) : (
                  ...
                  {items.map((n) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className={cn("text-sm", !n.read_at && "font-semibold text-brand-ink-900", n.read_at && "text-brand-ink-500")}>
                          {n.title}
                        </p>
                        {!n.read_at && (
                      ...
                      <p className="mt-0.5 text-xs text-brand-ink-400">{n.body}</p>
                      <p className="mt-1 text-[10px] text-brand-ink-300">
