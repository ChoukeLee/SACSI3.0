  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative rounded p-1.5 text-slate-500 transition hover:bg-slate-100 hover:text-brand-ink"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-md border border-black/10 bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-black/10 px-4 py-3">
              <h4 className="text-sm font-bold text-brand-ink">
                {t.title}
                {unread > 0 && (
                  <span className="ml-1.5 text-xs font-normal text-red-500">({unread} {t.unread})</span>
                )}
              </h4>
                  ...
                  {items.map((n) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className={cn("text-sm", !n.read_at && "font-semibold text-brand-ink", n.read_at && "text-slate-600")}>
                          {n.title}
                        </p>
                        {!n.read_at && (
