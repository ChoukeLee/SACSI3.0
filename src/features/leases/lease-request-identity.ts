...
export async function leaseRequestIdentity(
  operation: string,
  target: string,
  payload: unknown,
): Promise<string> {
  const key = `sacsi:lease-request:v1:${operation}:${target}`;
...
export function clearLeaseRequestIdentity(operation: string, target: string) {
  sessionStorage.removeItem(`sacsi:lease-request:v1:${operation}:${target}`);
}
