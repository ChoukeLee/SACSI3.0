...
export function DesktopSidebar({ locale, userRole }: { locale: Locale; userRole?: UserRole }) {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-sticky hidden w-60 border-r border-white/10 bg-[#1E1F2C] lg:flex lg:flex-col">
      {/* Brand */}
      <div className="flex h-16 shrink-0 items-center gap-3 px-5">
        <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-brand-indigo shadow-lifted">
          <span className="text-xs font-bold text-white">S</span>
        </div>
        <div className="leading-tight">
          <p className="text-xs font-bold uppercase tracking-[0.14em] text-white/50">SACIS 3.0</p>
          <h1 className="text-sm font-bold tracking-tight text-white">{labels.brand}</h1>
        </div>
      </div>
      ...
      <nav className="flex-1 space-y-5 overflow-auto px-3 py-4" aria-label="主导航">
        {visibleGroups.map(group => (
          <div key={group.key}>
            <p className="mb-1.5 px-3 text-xs font-bold uppercase tracking-[0.1em] text-white/40">
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      "active:scale-[0.98] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-indigo",
                      active
                        ? "bg-brand-indigo-500 text-white shadow-lifted"
      ...
      <div className="shrink-0 border-t border-white/10 px-5 py-3">
        <p className="text-xs font-semibold text-white/40">{labels.building}</p>
      </div>
    </aside>
  );
}
