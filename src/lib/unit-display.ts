...
export function getUnitOperationalLabel(
  unit: Pick<UnitRow, "notes" | "status" | "reservation_holder_name">,
  locale: "zh" | "fr",
): string | null {
  if (isOwnerOccupiedUnit(unit)) {
    return locale === "zh" ? "自用" : "Usage interne";
  }
  if (unit.status === "locked" && unit.reservation_holder_name?.trim()) {
    return unit.reservation_holder_name.trim();
  }
  return null;
}
