...
export default async function AssistantPage() {
  ...
  const conversation = await getOrCreateActiveConversation("zh");
  const history = await loadConversationHistory(conversation.id);
  return <AiWorkbenchView locale="zh" conversationId={conversation.id} initialHistory={history} canRecordFinance={hasPermission(user, "finance:write")} />;
}
