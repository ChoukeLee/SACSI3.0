...
export default async function FrenchDocumentsPage() {
  ...
  for (const lc of (leaseContracts ?? []) as LeaseContractRow[]) {
    ...
    docs.push({
      id: `lease_contract_${lc.id}`, docType: "lease_contract", source: "lease",
      title: `Contrat ${lc.contract_no}`, date: lc.start_date,
      unitNo: unit?.unit_no ?? "", customerName: cust?.name ?? "",
      customerPhone: cust?.phone ?? undefined, contractNo: lc.contract_no,
      amountXof: Number(lc.monthly_rent_xof), paidAmountXof: 0,
      status: lc.status, raw: lc,
    });
  }

  // ── Lease receivables → receipt + reminder ──
  for (const r of (receivables ?? []) as ReceivableRow[]) {
    ...
    if (Number(r.paid_amount_xof) > 0) {
      docs.push({
        id: `${idBase}_receipt`, docType: "lease_receipt", source: "lease",
        title: `Recu ${r.title}`, date: r.due_date,
        unitNo: unit?.unit_no ?? "", customerName: cust?.name ?? "",
        customerPhone: cust?.phone ?? undefined,
        amountXof: Number(r.amount_xof), paidAmountXof: Number(r.paid_amount_xof),
        status: r.status, raw: { ...r, receipt_no: null },
      });
    }

    if (unpaid > 0 && (r.status === "overdue" || r.due_date < new Date().toISOString().slice(0, 10))) {
      docs.push({
        id: `${idBase}_reminder`, docType: "lease_reminder", source: "lease",
        title: `Rappel ${r.title}`, date: r.due_date,
        unitNo: unit?.unit_no ?? "", customerName: cust?.name ?? "",
        customerPhone: cust?.phone ?? undefined,
        amountXof: Number(r.amount_xof), paidAmountXof: Number(r.paid_amount_xof),
        status: "overdue", raw: r,
      });
    }
  }
  ...
  for (const p of (payments ?? []) as PaymentRow[]) {
    ...
    docs.push({
      id: `payment_${p.id}`, docType: "lease_receipt", source: "lease",
      title: p.source_type === "lease_deposit" ? "Recu caution" : "Recu loyer",
      date: p.payment_date,
      unitNo: unit?.unit_no ?? "", customerName: cust?.name ?? "",
      customerPhone: cust?.phone ?? undefined,
      amountXof: Number(p.amount), paidAmountXof: Number(p.amount),
      status: "paid", raw: { ...p, receipt_no: p.receipt_no, contract_no: null },
    });
  }
  ...
  for (const db of (dailyBookings ?? []) as DailyBookingRow[]) {
    ...
    const nights = db.check_out
      ...
      : 1;
    docs.push({
      id: `daily_booking_${db.id}`, docType: "daily_booking", source: "daily",
      title: `Reservation ${unit?.unit_no ?? ""} ${db.check_in}`,
      date: db.check_in,
      unitNo: unit?.unit_no ?? "", customerName: cust?.name ?? "",
      customerPhone: cust?.phone ?? undefined,
      amountXof: Number(db.total_amount_xof), paidAmountXof: Number(db.prepaid_amount_xof),
      status: db.status, raw: { ...db, nights },
    });
  }
  ...
  for (const p of (payments ?? []) as PaymentRow[]) {
    ...
    const cust = c(p.customer_id);
    docs.push({
      id: `daily_payment_${p.id}`, docType: "daily_receipt", source: "daily",
      title: "Recu journalier",
      date: p.payment_date,
      unitNo: unit?.unit_no ?? "", customerName: cust?.name ?? "",
      customerPhone: cust?.phone ?? undefined,
      amountXof: Number(p.amount), paidAmountXof: Number(p.amount),
      status: "paid", raw: { ...p, receipt_no: p.receipt_no },
    });
  }
  ...
  for (const db of (dailyBookings ?? []) as DailyBookingRow[]) {
    if (db.status === "checked_out" || db.actual_check_out) {
      ...
      const cust = c(db.customer_id);
      docs.push({
        id: `daily_checkout_${db.id}`, docType: "daily_checkout", source: "daily",
        title: `Decompte depart ${unit?.unit_no ?? ""}`,
  ...
  for (const sc of (saleContracts ?? []) as SaleContractRow[]) {
    ...
    docs.push({
      id: `sale_contract_${sc.id}`, docType: "sale_contract", source: "sale",
      title: `Contrat vente ${sc.contract_no}`, date: sc.signed_date,
      unitNo: unit?.unit_no ?? "", customerName: cust?.name ?? "",
      customerPhone: cust?.phone ?? undefined, contractNo: sc.contract_no,
      amountXof: Number(sc.total_amount_xof), paidAmountXof: 0,
      status: sc.status, raw: sc,
    });
  }
  ...
  for (const p of (payments ?? []) as PaymentRow[]) {
    ...
    const cust = c(p.customer_id);
    docs.push({
      id: `sale_payment_${p.id}`, docType: "sale_receipt", source: "sale",
      title: "Recu vente",
      date: p.payment_date,
      unitNo: unit?.unit_no ?? "", customerName: cust?.name ?? "",
      customerPhone: cust?.phone ?? undefined,
  ...
  // ── Sale receivables with payments ──
  for (const r of (receivables ?? []) as ReceivableRow[]) {
    ...
    const cust = c(r.customer_id);
    docs.push({
      id: `sale_receivable_${r.id}`, docType: "sale_receipt", source: "sale",
      title: `Recu ${r.title}`, date: r.due_date,
      unitNo: unit?.unit_no ?? "", customerName: cust?.name ?? "",
      customerPhone: cust?.phone ?? undefined,
      amountXof: Number(r.amount_xof), paidAmountXof: Number(r.paid_amount_xof),
