...
async function FrenchLeasesData() {
  ...
  if (buildingIds.length > 0) {
    const [contractsRes, unitsRes, customersRes, paymentsRes, receivablesRes] = await Promise.all([
      ...
      supabase.from("payments").select("*").in("source_type", ["lease_rent", "lease_deposit", "lease_contract", "property_fee", "lease_agency_income", "lease_agency_expense"]).order("payment_date", { ascending: false }).limit(1000),
