...
import { parseReceiptRevisionInstruction } from "@/features/ai-workbench/receipt-revision-parser";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: job, error: jobError } = await supabase.from("ai_jobs").select("id, conversation_id").eq("id", proposal.job_id).single();
    ...
    const patch = parseReceiptRevisionInstruction(instruction);
    ...
    const assistantText = `草稿已更新至 v${version}：${built.match.building} ${built.match.roomNo}，${built.fields.amountXof} XOF，仍需人工确认后才会入账。`;
    await appendConversationTurn({
      ...
      context: { buildingCode: built.fields.buildingCode, unitNo: built.fields.roomNo, domain: "lease", proposalId, proposalVersion: version, changedFields },
    });
