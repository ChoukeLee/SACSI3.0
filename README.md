...
自然语言架构正在从“规则优先的单意图分类”迁移到“模型规划、安全工具执行、结果综合”。Query Plan V2 已进入默认关闭的影子评估阶段：它能表达多工具只读计划、结构化时间和澄清问题；统一只读工具执行器已支持精确日期区间和客户筛选，但尚未接入生产回答。当前上下文理解仍是受控字段和有限指代；Excel/PDF 批量导入、语音、合同导入、经营洞察和后台自动化仍在后续范围。
