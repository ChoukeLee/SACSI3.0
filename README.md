...
已完成自然语言只读查询、L1 保洁草稿确认执行、AI 证据链、持久化会话与近期对话恢复，以及从 AI 主输入框提交图片/文字凭证、继续用自然语言修改凭证草稿、人工确认与入账。凭证草稿每次修改都会重新读取业务记录、递增版本并写入审计事件。当前上下文理解仍是受控字段和有限指代，不等同于开放式通用对话；Excel/PDF 批量导入、语音、合同导入、经营洞察和后台自动化仍在后续范围。
